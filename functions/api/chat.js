...
const VECTORIZE_TOP_K_YEAR_QUERY = 50; // 問題內含年份時擴大候選池，避免同主題的舊年份文章擠掉正確年份
const YEAR_RE = /\b(20\d{2})\b/;
const SIMILARITY_THRESHOLD = 0.55; // 見 Phase 5 驗收：真正相關 ≥0.62，離題查詢 ≤0.55
// 逐年更新的主題（節慶/國定假日/旺季攻略）常常同時存在好幾個年份的相似文章，
// 彼此語意互相稀釋分數，導致正確年份的文章原始相似度低於一般門檻。
// 標題明確對到問題提到的年份，比絕對相似度分數更可靠，門檻可以放寬。
const YEAR_MATCH_THRESHOLD = 0.45;
...
// 留給 system prompt 其餘骨架（角色設定/知識補丁/引用格式說明）約 800-1000 tokens 空間，
// 讓整體 context 落在規格書講的「單次 context 上限約 4000 tokens」內。
const CONTEXT_TOTAL_CHAR_BUDGET = 3200;
...
export async function retrieveChunks(env, vector, message) {
  ...
  const isSameYear = m => queryYear && (m.metadata?.title || '').includes(queryYear);
  let matches = (result.matches || []).filter(m => m.score >= (isSameYear(m) ? YEAR_MATCH_THRESHOLD : SIMILARITY_THRESHOLD));
...
export function buildRagContext(matches, lang) {
  let budget = CONTEXT_TOTAL_CHAR_BUDGET;
  const parts = [];
  for (let i = 0; i < matches.length; i++) {
    if (budget <= 0) break;
    const m = matches[i];
    const url = localizeUrl(m.metadata.url, lang);
    const text = String(m.metadata.text || '').slice(0, Math.min(CHUNK_CHAR_CAP, budget));
    budget -= text.length;
    parts.push(`[來源${i + 1}] ${m.metadata.title}(${url})\n${text}`);
  }
  return parts.join('\n\n');
}
